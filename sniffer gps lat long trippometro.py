...
BAUDRATE = 9600            # 99 % dei GPS USB/NMEA usano 9600
SOURCE   = 'COM4'         # porta fisica del ricevitore GPS
GPS_OUT  = 'COM100'        # verso software di navigazione
STATS_PT = 'COM101'        # verso software di telemetria/statistiche
...
class GPSTracker:
    def __init__(self):
        ...
        # apriamo UNA volta le porte, poi condividiamo gli handle
        try:
    ...
    def _update_stats(self, lat, lon):
        ...
        self.last_pos=(lat,lon); self.last_t=now
        ...
        while self.running:
            try:
                ...
                if raw:
                    ...
                    self.gps_q.put(raw)
                    # parse solo GGA (basta un record completo)
                ...
                print(f"[reader] {e}"); self.running=False
            time.sleep(0.005)
        ...
        while self.running:
            try:
                while not self.gps_q.empty():
                    self.ser_gps.write(self.gps_q.get_nowait())
            except Exception as e:
                print(f"[gps_writer] {e}"); self.running=False
            time.sleep(0.005)
        ...
        while self.running:
            try:
                ...
                while not self.stats_q.empty():
                    self.ser_stats.write(self.stats_q.get_nowait().encode())

                # lettura comandi
                cmd=self.ser_stats.read_all().decode().strip().upper()
                if 'R' in cmd:
                    self.trip_dist=0; self.trip_speeds=[]
                    self.stats_q.put("TRIP RESET\n")
            except Exception as e:
                print(f"[stats_srv] {e}"); self.running=False
            time.sleep(0.1)

    # ---------- API ----------
    def start(self):
        print("Tracker avviato. Stats su COM101, dati GPS mirroring su COM100.")
        self.t_read.start(); self.t_write.start(); self.t_stat.start()
        try:
            while self.running:
                time.sleep(1)
